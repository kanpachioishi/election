...
const districtsPath = path.join(repoRoot, "data", "v1", "prefectural_assembly_districts.json");
const outputPath = path.join(repoRoot, "site", "pages", "prefectural-assembly-districts.html");
...
function renderRows(records) {
  return records
    ...
    .map((record) => [
      `            <tr data-pref-code="${escapeHtml(record.pref_code)}" data-prefecture="${escapeHtml(record.prefecture_name)}" data-seat-count="${escapeHtml(record.seat_count)}">`,
      `              <td>${escapeHtml(record.prefecture_name)}</td>`,
      `              <td>${escapeHtml(record.district_name)}</td>`,
      `              <td>${escapeHtml(record.area_label ?? "")}</td>`,
...
function renderPage(data) {
  ...
  const prefectureOptions = renderPrefectureOptions(data.records);
  const coveredPrefectures = data.coverage?.prefecture_count ?? new Set(data.records.map((record) => record.pref_code)).size;
  const districtCount = data.coverage?.district_count ?? data.records.length;
  const seatCount = data.coverage?.seat_count ?? data.records.reduce((sum, record) => sum + record.seat_count, 0);
...
<html lang="ja">
  <head>
    ...
    <title>県議選挙区・定数台帳 - わたしの選挙</title>
    <meta name="description" content="都道府県議会議員選挙の選挙区名、区域、定数を確認するための台帳です。">
  ...
  <body>
    <header class="site-header">
      ...
      <nav class="site-nav" aria-label="主なナビゲーション">
        ...
        <a href="prefectural-assembly-districts.html" aria-current="page">県議選挙区台帳</a>
    ...
    <main>
      <h1>県議選挙区・定数台帳</h1>
      <p>都道府県議会議員選挙の選挙区名、区域、定数を確認するための台帳です。初版は、非統一または個別管理が必要な県都を優先しています。</p>
      ...
      <section class="summary-grid" aria-label="台帳概要">
        ...
        <div class="summary-item">
          <span>選挙区</span>
          <strong>${formatNumber(districtCount)}区</strong>
        </div>
        <div class="summary-item">
          <span>定数合計</span>
          <strong>${formatNumber(seatCount)}人</strong>
        </div>
        <div class="summary-item">
          <span>生成日</span>
      ...
      <section class="note-panel">
        <p>${escapeHtml(coverageNote)} <a href="prefectural-assembly-terms.html">県議会任期台帳</a> と照合し、県ごとの選挙区数・定数合計が一致するよう検証しています。</p>
      </section>

      <div class="filter-bar">
        <div class="filter-field wide">
          <label for="searchInput">キーワード</label>
          <input id="searchInput" type="search" placeholder="例: 水戸、仙台市、島部">
        </div>
...
${prefectureOptions}
      ...
      <p class="count-note" id="countNote">${formatNumber(districtCount)}件表示</p>
...
${rows}
    ...
    <script>
      const rows = Array.from(document.querySelectorAll("#districtRows tr"));
      ...
      function applyFilters() {
        ...
        for (const row of rows) {
          ...
          const visible = matchesPrefecture && matchesKeyword;
          row.hidden = !visible;
...
async function main() {
  ...
  const data = await readJson(districtsPath);
  const output = renderPage(data);

  if (args.check) {
    const current = await fs.readFile(outputPath, "utf8");
    if (current !== output) {
      throw new Error("site/pages/prefectural-assembly-districts.html differs from data/v1/prefectural_assembly_districts.json");
    }
    return;
  }
