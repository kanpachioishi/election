(function () {
  ...
  const ELECTION_ID = "el-mun-10209-mayor-2026";
...
function renderHero(election, officialResources, signals, endorsements) {
  ...
  function renderOfficialSection(election, officialResources) {
    ...
    [
      ["投票日", formatDateWithWeekday(election.voteDate)],
      ["告示日", formatDateWithWeekday(election.noticeDate)],
      ["対象", election.primaryRegionShortName || election.primaryRegionName || "藤岡市"]
    ].forEach(([label, value]) => {
  ...
  function renderTimelineSection(election, officialResources) {
    ...
    if (election.noticeDate) {
      timelineItems.push({
        ...
        sourceName: officialSource ? officialSource.title : `${election.primaryRegionShortName || election.primaryRegionName || "藤岡市"}公式`,
  ...
  if (!election) {
    renderMissing("window.ELECTION_SITE_DATA から el-mun-10209-mayor-2026 を取得できませんでした。");
    return;
  }
